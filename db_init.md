...
> 云函数已支持自动创建缺失集合，但首次部署仍建议先初始化并准备管理员账号，避免出现权限相关问题。
...
> 注意：调用前请确保当前小程序用户在 `users` 集合中存在且 `isAdmin: true`。如果是全新部署，可先通过注册功能自动创建 `users` 集合，再在控制台补充管理员记录，或直接手动创建 users 集合和管理员记录后再调用。
