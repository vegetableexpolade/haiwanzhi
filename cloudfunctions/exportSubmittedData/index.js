...
function dateFilter(startDate, endDate) {
  ...
  if (startDate && endDate) {
    filter.submittedAt = _.gte(new Date(`${startDate}T00:00:00`)).and(_.lte(new Date(`${endDate}T23:59:59`)))
  } else if (startDate) {
    filter.submittedAt = _.gte(new Date(`${startDate}T00:00:00`))
  } else if (endDate) {
    filter.submittedAt = _.lte(new Date(`${endDate}T23:59:59`))
  }
  return filter
}
...
async function listAll(collectionName, filter) {
  ...
  const pageSize = 100
  while (true) {
    const query = db.collection(collectionName).where(filter).orderBy('submittedAt', 'desc').skip(page * pageSize).limit(pageSize)
...
exports.main = async (event) => {
  await requireAdmin()
  const { kind = 'all', startDate = '', endDate = '' } = event || {}
  const filter = dateFilter(startDate, endDate)
  const workbook = XLSX.utils.book_new()
  let exportedSheets = 0

  if (kind === 'scout' || kind === 'all') {
    const scoutItems = await listAll('scoutSegments', filter)
    ...
    appendSheet(workbook, scoutItems.map((item) => createCoastlineRow(item, ((item.entries || {}).coastline) || {})), '海岸线类型')
  ...
  if (kind === 'survey' || kind === 'all') {
    const surveyItems = await listAll('surveyStations', filter)
